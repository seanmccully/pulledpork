...
class SnortRuleParser:
    ...
    def __init__(self, log_file: str, rule_dirs: List[str] = None):
        ...
        re.compile(
            r'ERROR:\s+(\S+):(\d+) file type: rule id (\d+) found duplicate'
        )]
        ...
        try:
            with open(self.log_file, 'r') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    for dup_pattern in self.duplicate_pattern:
                        dup_match = dup_pattern.search(line.strip())
                        if dup_match:
                            if len(dup_match.groups()) > 3:
