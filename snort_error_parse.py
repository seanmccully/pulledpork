...
class SnortRuleParser:
    ...
    def handle_duplicate_rules(
        self, duplicate_errors: List[Dict], dry_run: bool = False
    ):
        """
        ...
        for error in duplicate_errors:
            ...
            for rule in found_rules:
                ...
                print(
                    f"    {status} Rev {rule['revision']}: {os.path.basename(rule['file_path'])}:{rule['line_num']}"
                )

            # Keep the highest revision (first in sorted list)
            newest_rule = found_rules[0]
            print(
                f"  Keeping newest revision {newest_rule['revision']} from {os.path.basename(newest_rule['file_path'])}"
            )
            ...
            for rule in found_rules[1:]:
                if not rule["is_commented"]:
                    ...
                    print(
                        f"  Disabling older rev {rule['revision']} in {file_name}:{rule['line_num']}"
                    )
...
Examples:
  ...
  python snort_parser.py snort.log -r /etc/snort/rules /opt/snort/rules --fix-duplicates
    ...
    parser.add_argument("log_file", help="Path to the Snort log file to parse")
