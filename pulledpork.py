...
def main():
    ...
    # Loading from the Snort rulesets?
        ...
        if conf.registered_ruleset:
            version = sub(r'[^a-zA-Z0-9]', '', '3.9.0.0')  # version in URL is alphanumeric only
...
def download_blocklists(conf):
    ...
    if conf.defined('blocklist_path') and any([conf.snort_blocklist, conf.et_blocklist, len(conf.blocklist_urls)]):
        ...
        blocklist_header += '# To Use this file, in your snort.lua, you need the following settings:\n'
...
def print_operational_settings():
    ...
    # env. variables
    log.verbose(f"The Snort version number used for processing is:  {conf.snort_version}")
    ...
    if conf.args.file:
        ...
        log.verbose(f"Rulesets will not be downloaded, they will be loaded from all files in local folder: \n\t {conf.args.folder}")
    else:
    ...
    if conf.rule_mode == 'policy':
        log.verbose(f"the policy file written (to specify enabled rules) is:  {conf.policy_path}")
    ...
    # reload snort
    if conf.defined('pid_path'):
        log.verbose(f"Snort will be reloaded with new configuration, Pid loaded from:  {conf.pid_path}")
    else:
...
def compile_so_rules(src_path, dst_path):
    ...
    try:
        ...
        process = Popen('make clean && make', stdout=PIPE, stderr=PIPE, shell=True,
                       universal_newlines=True, cwd=src_path, env=env)
        output, error = process.communicate()
